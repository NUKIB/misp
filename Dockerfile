...
COPY su-exec.c /tmp/
RUN dnf install -y --setopt=tsflags=nodocs --setopt=install_weak_deps=False gcc-toolset-15 make && \
...
RUN --mount=type=tmpfs,target=/var/cache/dnf \
    ...
    pip3.14 --no-cache-dir install --disable-pip-version-check -r /tmp/requirements.txt && \
    mkdir /run/php-fpm && \
    rm -rf /tmp/packages
